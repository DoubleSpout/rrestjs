...
var AsyncProxy = require('./modules/AsyncProxy'),
    ...
    staticObj = {
        ...
        genless: function(filepath, cb) { //生成编译后的less文件，即是在原来css文件加前缀less.
            var cb = cb ||
            function() {}, fileary = filepath.split('/'), filedir = fileary.slice(0, fileary.length - 1).join('/');
        ...
        parse: function(type, filePathArray, md5str, callback) {
            var as = AsyncProxy(),
                ...
                //存放异步方法的数组
                fileary = [],
                //存放buffer.toString()的css或js文档
                wrong = 0,
                //错误记录，大于0说明有错误
                genlessnum = filePathArray.length,
                //待生成less的文件数
                gencount = 0,
                //已经生成的less文件数
                readfile = function(value, index) {
                var //value = value.replace(autoStatic, ''),
                    //静态文件路径去掉前缀
                    staticPath = staticFolder + value,
                    //文件的绝对路径
                    recfn = function(rec) {
