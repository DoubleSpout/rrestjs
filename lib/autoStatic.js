	...
	staticObj = {
		...
		autoStaticFn:function(req, res){
			...
			if(lessType === 'less'){//如果是css文件则去生成less.前缀的css文件名
				staticObj.genless(filepath.replace('.css', '.less'), function(err, csspath){//生成编译后的less的css文件，返回err和csspath两个参数
		...
		getLessType:function(path){
			if(~path.indexOf('.css')) return 'less';
			return false;
        },
		genless:function(filepath, cb){//生成编译后的less文件，即是在原来css文件加前缀less.			
			...
			fs.readFile(filepath, 'utf-8',function(err, data){//根据请求读取目录
                ...
                new(less.Parser)({
					...
					var lesspath = filepath.replace('.less', '.css');//根据元有文件路径拼装成带less.为前缀的less css文件
					fs.writeFile(lesspath, tree.toCSS(), function(err){//根据 lesspath 生成编译好的css文件
		...
		parse:function(type, filePathArray, md5str, callback){
			var as = AsyncProxy(),//实例化异步代理库 AsyncProxy
				...
				readfile = function(value, index){
					...
					if(staticObj.getLessType(value) === 'less'){
				...
				filePathArray.forEach(function(value, index){//为每一个路径添加fs访问路径
					...
					if(vtype !=='css' && vtype !== 'js')  return wrong++;//如果后面的文件不是css也不是js
					ary.push(readfile(value, index));					
				});	
				if(wrong>0) return callback(msg.resmsg.parseError);
				ary.push(allComplete);	
				if(staticObj.getLessType(filePathArray[0]||'') !== 'less') as.ap.apply(as, ary);
			return true;				
		},
