...
var normalize_query = restUtils.normalize_query,
 ...
 * @param {Response} res
 */
staticObj.autoStaticFn = function(req, res) {
    // 拼装静态文件绝对地址
 ...
 */
staticObj.getMimeType = function(filePath) { 
    var po = filePath.lastIndexOf('.') + 1;
    return filePath.slice(po).toLowerCase();
};
 ...
 */
staticObj.isLessType = function(cssPath) {
    return (/\.less(?:\.css)?$/).test(cssPath);
};
/**
 ...
 */
staticObj.genLess = function(filePath, callback) { 
    var filePos = filePath.lastIndexOf('/');
        fileDir = filePath.slice(0, filePos);
    ...
    fs.readFile(filePath, 'utf-8', function(err, data) { //根据请求读取目录
        ...
        new(less.Parser)({
            ...
            var fileName = filePath.slice(filePos + 1),
                lessPath = filePath.replace(fileName, 'compile.' + fileName); //根据元有文件路径拼装成带less.为前缀的less css文件
 ...
 * @param {Function} callback
 */
staticObj.parse = function(type, filePathArray, md5str, callback) {
    var as = asyncProxy(), //实例化异步代理库 AsyncProxy
        ...
        readfile = function(value, index) {
            // 静态文件路径去掉前缀
            value = value.replace(autoStatic, ''); 
            var staticPath = staticFolder + value, //文件的绝对路径
                recfn = function(rec) {
    ...
    // 为每一个路径添加fs访问路径
    filePathArray.every(function(value, index) {
        // 如果访问路径不正确
        if (!reg.test(value)) {
            return wrong++ && false;
        }
 ...
 * @param {Response} res
 */
staticObj.createCache = function(type, filePathArray, md5str, res) { //生成缓存的方法
    // 生成缓存以后的回调，如果有错误也会执行
