...
//static
		statciMaxAge:function(val){
			if(!checknumber(val)) return errfn('statciMaxAge must be a number, for example:86400000*7 = 7 days');
			return true;
		},
...
//logger log4js
		...
		logPath:function(val){
			if(!checkpath(val)) return errfn('logPath must be a path, for example: /mylogs/console.log');
			return true;
		},		
