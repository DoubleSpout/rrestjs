...
if(_restConfig.isCluster){//开启集群则输出以下函数
			...
			var callback = callback || function(req, res){};
				cp = ClusterPlus({
						CreateCallback:function(err, data){
							if(!err) {
								_restConfig.pid = data.num;
								_rrest.id =  data.num;//将序号传递给work进程
								module.exports.restlog = restlog();//输出restlog的api
								readycb();//执行用户的回调函数
	...
	var readycb;//保存用户的回调函数
	module.exports.ready = function(cb){
			readycb = cb||function(){};//赋值用户的回调函数
		};
}
else{
	module.exports = entrance;//如果没有开启cluster集群则输出正常入口函数
	module.exports.ready = function(cb){cb&&cb();}
	module.exports.restlog = restlog();//输出restlog的api
