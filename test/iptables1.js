...
module.exports.rrestjsconfig = {
	...
	ExceptIP:new RegExp("^192.168.11.11$"), //正则表达式，匹配成功表示此IP可以正常访问,白名单
	ExceptPath:['/user'],//例外的路径，如果用户访问这个url路径，无论在不在ip过滤列表中，都可以正常使用，白名单才能使用
...
getfn('/user', function(res, body){
	...
	should.strictEqual(body, 'get it');
	result('white list except');
});
